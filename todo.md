...
2. On top put of ``llama.js``:
```
var mongoose = require('mongoose');
var Schema = mongoose.Schema;
```

3. Now set up schema and structure in ``llamas.js``:
```
...
4. In the ``.env`` file place ``MONGO_URI=mongodb://localhost/llamas`` on top... and pull the ``mongoose.connect`` to that file by writing ``process.env.MONGO_URI`` 
...
5. Let tell ``app.js`` that we need to connect to these routes (if all routes are in index then we don't need to add ``app.use('/api', llamas))``
...
2. **Testing** in terminal the **GET route** ``http GET http://localhost:3000/api/llamas``

3. Set up **POST route** now:
...
router.post('/llamas', function(req, res, next) {
 ...
 ```
**Testing** in terminal **POST route** in terminal:
...
9. Now update the get router **GET aLL router** 
    - mongoose find function ``(return res.json(llamas))``
```
router.get('/llamas', function(req, res, next) {
...
10. Helpful resource :  ``mongoosejs.com/docs/api.html
